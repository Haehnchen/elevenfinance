...
import {
  LEVERAGE_FETCH_BALANCES_BEGIN,
  LEVERAGE_FETCH_BALANCES_SUCCESS,
  LEVERAGE_FETCH_BALANCES_FAILURE,
} from './constants';

export function fetchBalances(data) {
  return dispatch => {
    dispatch({
      type: LEVERAGE_FETCH_BALANCES_BEGIN,
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      multicall
        ...
        .then(([results]) => {
          ...
          dispatch({
            type: LEVERAGE_FETCH_BALANCES_SUCCESS,
            data: newTokens,
          });
        ...
        .catch(error => {
          dispatch({
            type: LEVERAGE_FETCH_BALANCES_FAILURE,
          });
...
export function useFetchBalances() {
  ...
  const { tokens, fetchBalancesDone, fetchBalancesPending } = useSelector(
    state => ({
      tokens: state.leverage.tokens,
      fetchBalancesDone: state.leverage.fetchBalancesDone,
      fetchBalancesPending: state.leverage.fetchBalancesPending,
    }),
    shallowEqual,
  );
...
export function reducer(state, action) {
  ...
  switch (action.type) {
    case LEVERAGE_FETCH_BALANCES_BEGIN:
      return {
        ...state,
    ...
    case LEVERAGE_FETCH_BALANCES_SUCCESS:
      return {
        ...state,
        tokens: action.data,
    ...
    case LEVERAGE_FETCH_BALANCES_FAILURE:
      return {
        ...state,
        fetchBalancesPending: false,
      };
